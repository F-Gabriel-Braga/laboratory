...
function start() {
    ...
    setInterval(calculateFirePropagation, 20)
}
...
function updateFireIntensityPerPixel(currentPixelIndex) {
    ...
    const decay = Math.floor(Math.random() * 3) // Efeito do Fogo
    const belowPixelFireIntensity = firePixelsArray[belowPixelIndex]
    const newFireIntensity = (belowPixelFireIntensity - decay) >= 0 ? (belowPixelFireIntensity - decay) : 0

    firePixelsArray[currentPixelIndex - decay] = newFireIntensity // Efeito do Fogo + Vento
}
...
function renderFire() {
    ...
    for(let row = 0; row < fireHeight; row++) {
        html += '<tr>'
        for(let colunm = 0; colunm < fireHeight; colunm++) {
            const pixelIndex = colunm + (fireWidth * row)
